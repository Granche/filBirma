...
module.exports = {
  ...
  smasher: function(){
    ...
    var enBil = new Bil({
      model: this.modelArr[Math.round(Math.random()*this.modelArr.length)],
      damage: this.damageArr[Math.round(Math.random()*this.damageArr.length)],
      owner: this.ownerArr[Math.round(Math.random()*this.ownerArr.length)],
    });

    var usedRegs = [];
    var aReg = this.regArr[Math.round(Math.random()*this.regArr.length)];
    
    for(var i=0; i<usedRegs.length; i++) {
      if(aReg == usedRegs[i]){
        aReg = this.regArr[Math.round(Math.random()*this.regArr.length)];
      }
      continue    
    }
